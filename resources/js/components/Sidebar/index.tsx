...
import { Link } from '@inertiajs/react'; // No need for useLocation import anymore
import SidebarLinkGroup from './SidebarLinkGroup';
import Logo from '../../images/logo/logo.svg';
import { usePage } from '@inertiajs/react'; // Import usePage to access page URL
...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  console.log('working side bar');

  // Use usePage to access the current URL
  const { url } = usePage();

  // Create a full URL using window.location.origin to ensure the URL is absolute
  const { pathname } = new URL(url, window.location.origin);  // Corrected here

  console.log(pathname);  // Now it will work as expected
  ...
  console.log(url);  // Logs the full URL
  ...
  }, [sidebarOpen]); // Added dependency to update on sidebarOpen change
  ...
  // Save sidebarExpanded state to localStorage
  useEffect(() => {
